...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        printf("Usage: sws_q6 [server_directory]\n");
        return -1;
    } else {
    ...
    event.events = EPOLLIN; // event : data can be read
    ...
    // add timer_fd to the interest list
    event.data.fd = timer_fd;
    event.events = EPOLLIN; // event : data can be read
    if (epoll_ctl(epfd, EPOLL_CTL_ADD, timer_fd, &event) == -1) {
        errExit("timer_fd epoll_ctl");
    }
    
	// storage buffer for events
	events = calloc(MAX_EVENTS, sizeof(event));
	timers = calloc(EPOLL_SIZE, sizeof(timer));
		    ...
		    if ((events[i].events & EPOLLERR) ||
              ...
              // An error has occured or socket not ready
	            ...
	            while (1) {
                    ...
                    if (timerfd_settime (timer_fd, flags, &new_timer, &old_timer) == -1) {
                        close(client_fd);
	            ...
	            size_t sec = 0;
	            if (read(timer_fd, &sec, sizeof(sec)) != -1) {
